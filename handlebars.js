	...
	var schnauzer = this.schnauzer = new Schnauzer('', {
		tools: function(_this, findData, fn, name, params, data, parts, body, altBody) {
			...
			for (var n = params.length; n--;) {
				key = parts.rawParts[params[n]] || {
					value: params[n],
					keys: [params[n]],
					depth: 0,
				};
				hash[params[n]] = params[n] =
					key.isString ? key.value : findData(data, key.value, key.keys, key.depth);
			}

			var value = fn.apply(data.path[0], params.concat({
					name: name,
					hash: hash,
					fn: function(_data) { return body && (data.path[0] = _data) && body(data) },
					inverse: function(_data) {
						return altBody && (data.path[0] = _data) && altBody(data);
					},
